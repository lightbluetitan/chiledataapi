...
#'
#' This function retrieves the observed exchange rate ("Euro (pesos por euro)") in Chilean Pesos
#' from the API endpoint: \url{https://findic.cl/api/euro}. The data is provided by the
...
#'   \item \code{valor}: Valor del euro observado en pesos chilenos (numérico).
#' }
#'
#' @details
#' The function sends a GET request to the \code{/api/euro} endpoint. If the request is successful (HTTP 200),
...
#'
#' @export
get_chile_euro <- function() {
  url <- "https://findic.cl/api/euro"
